    ...
    <section id="section" class="row">
        <aside class="col-2" id="side1">
            <?php
				...
				$dane = @$polaczenie->query("SELECT nazwa, ranking FROM postac WHERE idkonta=$id_gracza");
				...
				if($ile_danych > 0) {
					...
					while($r = $dane->fetch_assoc()) {
						...
						echo $r['nazwa']." (".$r['ranking']." miejsce w rankingu)<br>";
					}
				} else {
					
					echo "<br>Na twoim koncie nie zostały jeszcze stworzone żadne postacie<br>";
				}
        ...
        <article class="col-6">
			...
			<?php
				...
				$dane1 = @$polaczenie->query("SELECT ranking, nazwa, poziom, login FROM postac JOIN konto On postac.idkonta = konto.id ORDER BY ranking");
				...
				if($ile_danych > 0) {
					...
					while($r = $dane1->fetch_assoc()) {
						...
						echo '<td><a href="postac.php?nazwa='.$r['nazwa'].'">'.$r['nazwa']."</a></td>";
						echo "<td>".$r['poziom']."</td>";
						echo "<td>".$r['login']."</td>";
						echo "</tr>";
					}
